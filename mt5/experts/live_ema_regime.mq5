//+------------------------------------------------------------------+
//|                                           live_ema_regime.mq5    |
//| Trades the latest M5 regime file written by the live engine.     |
//+------------------------------------------------------------------+
...
bool     g_fileOk = false;
string   g_lastFileError = "Waiting for regime file";
string   g_regimeFile = "";
...
bool ReadLatestRegime(LiveRegime &regime)
  {
   ...
   regime.confidence = StringToDouble(confidenceText);
   if(regime.time <= 0 || regime.name == "" || confidenceText == "")
     {
      g_fileOk = false;
      g_lastFileError = "Missing time/regime_name/confidence in " + g_regimeFile;
      return false;
     }
   return true;
  }
...
bool OpenTrade(int direction, double atr)
  {
   ...
   if(atr <= 0.0)
     {
      Print("Open skipped: ATR is unavailable or invalid: ", atr);
      return false;
     }
   ...
   bool opened = direction > 0
                  ? g_trade.Buy(InpLots, _Symbol, 0.0, sl, tp, "Regime BUY")
                  : g_trade.Sell(InpLots, _Symbol, 0.0, sl, tp, "Regime SELL");
   if(!opened)
     {
...
//+------------------------------------------------------------------+
bool GetAtrValue(datetime signalTime, double &value)
  {
   int shift = iBarShift(_Symbol, InpEmaTimeframe, signalTime, false);
   if(shift < 0)
      return false;
   ...
   if(CopyBuffer(g_atrHandle, 0, shift, 1, values) != 1 ||
     ...
     {
      Print("ATR unavailable for regime at ", TimeToString(signalTime),
            ". Error ", GetLastError());
      return false;
     }
   value = values[0];
...
void DrawFileErrorPanel()
  {
   ...
   SetBackground(PREFIX + "bg", x - 8, y - 8, 430, lineH * 3 + 14);
   SetLabel(PREFIX + "title", "EMA REGIME LIVE - FILE ERROR", x, y, clrRed, InpFontSize + 1);
   SetLabel(PREFIX + "l1", g_lastFileError, x, y + lineH, clrOrange);
   SetLabel(PREFIX + "l2", "Regime:   " + g_regimeFile, x, y + lineH * 2, clrSilver);
   for(int i = 3; i < 18; i++)
...
//+------------------------------------------------------------------+
void DrawPanel(const LiveRegime &regime)
  {
   if(!InpShowPanel)
      return;
   int x = InpXOffset, y = InpYOffset;
   ...
   long ageSeconds = (long)(TimeCurrent() - regime.time);
   ...
   double atr = 0.0, adxValue = 0.0;
   bool atrAvailable = GetAtrValue(regime.time, atr);
   bool adxAvailable = GetAdxValue(regime.time, adxValue);
   bool adxPassed = adxAvailable && adxValue > InpMinimumAdx;

   double ema9 = 0.0, ema21 = 0.0;
   bool emaAvailable =
      GetIndicatorValue(g_ema9Handle, InpEmaTimeframe, regime.time, "EMA(9)", ema9) &&
      GetIndicatorValue(g_ema21Handle, InpEmaTimeframe, regime.time, "EMA(21)", ema21);
   double previousEma9 = 0.0, previousEma21 = 0.0;
   bool previousEmaAvailable = emaAvailable &&
      GetPreviousIndicatorValue(g_ema9Handle, InpEmaTimeframe, regime.time,
                                "EMA(9)", previousEma9) &&
      GetPreviousIndicatorValue(g_ema21Handle, InpEmaTimeframe, regime.time,
                                "EMA(21)", previousEma21);
   double gapIncrease = previousEmaAvailable
                        ? MathAbs(ema9 - ema21) -
                          MathAbs(previousEma9 - previousEma21) : 0.0;
   double requiredGapIncrease = InpMinimumEmaGapIncreaseAtr * atr;
   ...
   int i = 0;
   SetLabel(PREFIX + "title", "EMA REGIME LIVE", x, y + lineH * i, clrWhite, InpFontSize + 1); i++;
   SetLabel(PREFIX + "l1", "Symbol:       " + _Symbol + "   [M5]", x, y + lineH * i, clrWhite); i++;
   SetLabel(PREFIX + "l2", "Regime Time:  " + TimeToString(regime.time, TIME_DATE | TIME_MINUTES) + "  (" + ageText + ")", x, y + lineH * i, ageCol); i++;
   SetLabel(PREFIX + "l3", "Regime:       " + regime.name, x, y + lineH * i, bullishRegime || bearishRegime ? clrLimeGreen : clrAqua); i++;
   ...
   SetLabel(PREFIX + "l6", "Confidence:   " + DoubleToString(regime.confidence, 6) + " > " + DoubleToString(InpMinimumConfidence, 2) + " [" + (confidencePassed ? "PASS" : "FAIL") + "]", x, y + lineH * i, confidencePassed ? clrLimeGreen : clrRed); i++;
   ...
   SetLabel(PREFIX + "l11", "Gap +" + DoubleToString(InpMinimumEmaGapIncreaseAtr, 2) + "ATR: " + (gapPassed ? "TRUE" : "FALSE") + (previousEmaAvailable && atrAvailable ? "  (" + DoubleToString(gapIncrease, digits) + " / " + DoubleToString(requiredGapIncrease, digits) + ")" : ""), x, y + lineH * i, gapPassed ? clrLimeGreen : clrRed); i++;
   ...
   SetLabel(PREFIX + "l16", "ATR:          " + (atrAvailable ? DoubleToString(atr, digits) : "unavailable") + "   Source: " + g_regimeFile, x, y + lineH * i, atrAvailable ? clrSilver : clrRed); i++;
   ChartRedraw();
  }

//+------------------------------------------------------------------+
void ProcessLatestRegime()
  {
   LiveRegime regime;
   ...
   double atr = 0.0;
   if(!GetAtrValue(regime.time, atr))
      return;

   if(!EmaPassed(regime.time, desiredDirection, atr))
      return;

   if(!AdxPassed(regime.time))
      return;
   ...
   if(!IsWithinEntryHours())
      return;
   OpenTrade(desiredDirection, atr);
  }
...
int OnInit()
  {
   ...
   Print("Chart symbol ", _Symbol, ": regime file=", g_regimeFile);
