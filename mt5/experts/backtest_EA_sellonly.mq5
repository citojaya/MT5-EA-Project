//+------------------------------------------------------------------+
//|                                      backtest_EA_sellonly.mq5    |
//|   Short/flat XAUUSD regime strategy. Short in regimes 1,2,5,7;  |
...
input ulong            InpMagicNumber     = 26071702;            // Magic number
...
//+------------------------------------------------------------------+
//| Regime set used by this sell-only mirror strategy                |
//+------------------------------------------------------------------+
bool IsShortRegime(string regime)
  {
   return regime == "1" || // Weak Bull Trend
          regime == "2" || // Strong Bear Trend
...
bool ClosePositionOnRegimeChange(string regime, string signalTime)
  {
   ...
   if(!IsShortRegime(regime))
     {
      CloseManagedPosition("Regime " + regime + " is outside short set {1,2,5,7}");
...
//+------------------------------------------------------------------+
//| Place a sell based on the eligible regime signal                 |
//+------------------------------------------------------------------+
void ProcessTradingSignal()
  {
   ...
   bool sellSignal = IsShortRegime(regime);
   if(!sellSignal)
      return;
   ...
   double sl = InpStopLossAtr > 0.0 ? NormalizeDouble(bid + InpStopLossAtr * atr, digits) : 0.0;
   double tp = InpTakeProfitAtr > 0.0 ? NormalizeDouble(bid - InpTakeProfitAtr * atr, digits) : 0.0;
   bool opened = g_trade.Sell(InpLots, _Symbol, bid, sl, tp, "Short regime " + regime);
...
void DrawPanel()
  {
   ...
   bool shortRegime = IsShortRegime(regimeNum);
   string strategyState = shortRegime ? "SHORT" : "FLAT";
   color strategyColor = shortRegime ? clrRed : clrSilver;
   ...
   SetLabel(PREFIX + "l7", "Strategy:    " + strategyState + " (short set: 1,2,5,7)", x, y + lineH * i, strategyColor); i++;
