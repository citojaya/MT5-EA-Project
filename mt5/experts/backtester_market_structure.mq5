   ...
   if(g_isTester)
     {
      ...
      if(selected < 0 || g_csvSignals[selected].time != completedM1Time)
         return false;
...
void PlaceManualButtonOrder(const bool buyOrder)
  {
   TrendDirection requestedDirection = buyOrder ? TREND_UP : TREND_DOWN;
   if(!TradeDirectionAllowed(requestedDirection))
     {
      Print("Manual ", buyOrder ? "BUY" : "SELL",
            " blocked by InpTradeDirectionMode");
      return;
     }

   if(!g_stateAvailable)
     {
      Print("Manual order blocked: EMA(21) is not available");
      return;
     }
   ...
   double entry = buyOrder ? SymbolInfoDouble(_Symbol, SYMBOL_ASK)
                           : SymbolInfoDouble(_Symbol, SYMBOL_BID);
   bool emaPassed = buyOrder ? entry > g_state.fastEma
                             : entry < g_state.fastEma;
   if(!emaPassed)
     {
      Print("Manual ", buyOrder ? "BUY" : "SELL",
            " blocked by EMA(21): price=",
            DoubleToString(entry, _Digits), ", EMA21=",
            DoubleToString(g_state.fastEma, _Digits));
      return;
     }
   bool h1AtrPassed = buyOrder ? entry > g_state.h1AtrTrailingStop
                               : entry < g_state.h1AtrTrailingStop;
   if(!h1AtrPassed)
     {
      Print("Manual ", buyOrder ? "BUY" : "SELL",
            " blocked by H1 ATR line: price=",
            DoubleToString(entry, _Digits), ", H1 ATR line=",
            DoubleToString(g_state.h1AtrTrailingStop, _Digits));
      return;
     }
   ...
   bool opened = buyOrder
                 ? g_trade.Buy(volume, _Symbol, 0.0, 0.0, takeProfit,
                               "Manual button BUY")
                 : g_trade.Sell(volume, _Symbol, 0.0, 0.0, takeProfit,
                                "Manual button SELL");
   if(!opened)
...
void CreateTradeButtons()
  {
   if(!InpShowPanel)
      return;
   int y = InpYOffset + (InpFontSize + 8) * 14;
...
void DrawPanel()
  {
   ...
   SetBackground(PREFIX + "bg", x - 8, y - 8, 560, lineHeight * 16 + 14);
   CreateTradeButtons();
