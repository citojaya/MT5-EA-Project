//+------------------------------------------------------------------+
//|                              backtester_market_strucutre.mq5    |
//| Strategy Tester version of the live market-structure EA.        |
//+------------------------------------------------------------------+
...
input ENUM_TIMEFRAMES InpSignalTimeframe = PERIOD_M5; // Selectable entry timeframe
...
input color  InpEma21Color = clrYellow;               // EMA(21) chart colour
input color  InpEma50Color = clrDodgerBlue;           // EMA(50) chart colour
input color  InpEma100Color = clrMagenta;             // EMA(100) chart colour
input double InpMinimumEmaGapAtr = 1.5;               // Minimum gap between adjacent EMAs in ATR
...
const bool   InpUseRegimeCsv = false;                 // External regime input disabled
...
input double InpTakeProfitAtrMultiplier = 5.0;         // Take-profit distance in ATR
...
struct StructureState
  {
   ...
   double previousAtrTrailingStop;
   double candleOpen;
   double close;
...
bool     g_buyAtrSideConsumed = false;
bool     g_sellAtrSideConsumed = false;
...
bool CalculateAtrTrailingStop(const ENUM_TIMEFRAMES timeframe,
                              ...
                              double &previousClosedStop)
  {
   ...
   previousClosedStop = EMPTY_VALUE;
   for(int index = 0; index < ratesCount - 1; index++)
     {
      ...
      if(index == 0)
         ...
         stop = rates[index].close > previousStop
                ...
                : rates[index].close + loss;
      previousStop = stop;
      if(index == ratesCount - 3)
         previousClosedStop = stop;
     }
   return previousClosedStop != EMPTY_VALUE;
  }
...
bool AtrLineFilterPassed(const StructureState &state)
  {
   if(state.trend == TREND_UP)
      return state.close > state.fastEma &&
             ...
             state.close > state.atrTrailingStop;
...
//+------------------------------------------------------------------+
bool EmaGapFilterPassed(const StructureState &state)
  {
   if(state.atr <= 0.0)
      return false;

   double minimumGap = InpMinimumEmaGapAtr * state.atr;
   return MathAbs(state.fastEma - state.slowEma) > minimumGap &&
          MathAbs(state.slowEma - state.ema100) > minimumGap;
  }
...
string H1AtrAllowedDirectionText()
  {
   if(!g_stateAvailable || g_state.atrTrailingStop <= 0.0)
      return "WAITING";
   double price = g_state.close;
   if(price > g_state.fastEma &&
      g_state.fastEma > g_state.slowEma &&
      g_state.slowEma > g_state.ema100 && price > g_state.atrTrailingStop)
...
//+------------------------------------------------------------------+
bool UpdateRegimeFilter(const datetime cutoffTime)
  {
   ...
   if(g_isTester)
     {
      ...
      while(left <= right)
        {
         ...
         if(g_csvSignals[middle].time <= cutoffTime)
           {
            selected = middle;
            left = middle + 1;
           }
      ...
      g_currentCsvRegime = g_regimeFilterValue;
      g_currentCsvName = g_regimeFilterName;
      g_currentCsvConfidence = g_csvSignals[selected].confidence;
...
string TrendText(TrendDirection trend)
  {
   if(trend == TREND_UP)
      return "BUY - EMA STACK + " + TimeframeText() + " ATR";
   if(trend == TREND_DOWN)
      return "SELL - EMA STACK + " + TimeframeText() + " ATR";
...
bool CalculateStructure(StructureState &state)
  {
   ...
   if(InpUseAdxFilter &&
      ...
      return false;
   if(!CalculateAtrTrailingStop(InpSignalTimeframe, g_atrHandle,
                                state.atrTrailingStop,
                                state.previousAtrTrailingStop))
      return false;

   state.close = rates[1].close;
   state.candleOpen = rates[1].open;
   if(state.candleOpen <= 0.0)
      return false;
   ...
   state.detail = "EMA stack is not aligned with " + TimeframeText() + " ATR";
   if(state.close > state.fastEma && state.fastEma > state.slowEma &&
      state.slowEma > state.ema100 &&
      state.close > state.atrTrailingStop)
     {
      state.trend = TREND_UP;
      state.detail = "Close > EMA21 > EMA50 > EMA100; above " +
                     TimeframeText() + " ATR";
     }
   else if(state.close < state.fastEma && state.fastEma < state.slowEma &&
           state.slowEma < state.ema100 &&
           state.close < state.atrTrailingStop)
     {
      state.trend = TREND_DOWN;
      state.detail = "Close < EMA21 < EMA50 < EMA100; below " +
                     TimeframeText() + " ATR";
     }
   return true;
  }
...
//+------------------------------------------------------------------+
#ifdef REMOVED_CLOSE_RULES
void CheckProfitableM5AtrFlipExit(const StructureState &state)
  {
   bool flippedDown = state.previousClose >= state.previousAtrTrailingStop &&
                      state.close < state.atrTrailingStop;
   bool flippedUp = state.previousClose <= state.previousAtrTrailingStop &&
                    state.close > state.atrTrailingStop;
   long typeToClose = flippedDown ? POSITION_TYPE_BUY
                                  : flippedUp ? POSITION_TYPE_SELL : -1;
   if(typeToClose < 0)
      return;
   ...
   for(int index = PositionsTotal() - 1; index >= 0; index--)
     {
      ...
      if(!g_trade.PositionClose(ticket, InpDeviationPoints))
         Print("Profitable M5 ATR flip close failed for #", ticket, ": ",
               g_trade.ResultRetcodeDescription());
      else
         Print("Closed profitable position #", ticket,
               flippedDown ? " on bearish M5 ATR flip"
                           : " on bullish M5 ATR flip");
     }
  }
#endif
...
void CheckH1AtrDirectionExit()
  {
   if(!InpEnableH1AtrClose || !g_stateAvailable ||
      g_state.h1AtrTrailingStop <= 0.0)
      return;
...
//+------------------------------------------------------------------+
void MaintainAtrTrailingStopLoss()
  {
   if(!g_stateAvailable || g_state.atrTrailingStop <= 0.0)
      return;
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   double candidate = NormalizePrice(g_state.atrTrailingStop);
   ...
   for(int index = PositionsTotal() - 1; index >= 0; index--)
     {
      ...
      bool improve = type == POSITION_TYPE_BUY
                     ? candidate < bid &&
                       (currentStop == 0.0 || candidate > currentStop + point * 0.5)
                     : candidate > ask &&
                       (currentStop == 0.0 || candidate < currentStop - point * 0.5);
      if(improve && !g_trade.PositionModify(ticket, candidate, takeProfit))
         Print("ATR trailing stop update failed for position #", ticket, ": ",
               g_trade.ResultRetcodeDescription());
     }
  }

//+------------------------------------------------------------------+
bool AtrSideEntryAvailable(const TrendDirection direction)
  {
   if(direction == TREND_UP)
      return !g_buyAtrSideConsumed;
   if(direction == TREND_DOWN)
      return !g_sellAtrSideConsumed;
   return false;
  }

//+------------------------------------------------------------------+
void RearmEntriesAfterAtrSideSwap(const StructureState &state)
  {
   if(state.close < state.atrTrailingStop)
      g_buyAtrSideConsumed = false;
   else if(state.close > state.atrTrailingStop)
      g_sellAtrSideConsumed = false;
  }

//+------------------------------------------------------------------+
bool FindPreviousSwingStop(const TrendDirection direction, double &stopLoss)
  {
   int requested = MathMax(InpPivotLookback,
                           InpPivotLeft + InpPivotRight + 10);
   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   int copied = CopyRates(_Symbol, InpSignalTimeframe, 0, requested, rates);
   ...
   bool newerSwingAvailable = false;
   double newerSwingPrice = 0.0;
   for(int shift = InpPivotRight + 1;
       shift < copied - InpPivotLeft; shift++)
     {
      bool found = direction == TREND_UP
                   ? IsPivotLow(rates, copied, shift)
                   : IsPivotHigh(rates, copied, shift);
      if(!found)
         continue;

      double swingPrice = direction == TREND_UP
                          ? rates[shift].low : rates[shift].high;
      if(!newerSwingAvailable)
        {
         newerSwingPrice = swingPrice;
         newerSwingAvailable = true;
         continue;
        }

      bool structureConfirmed = direction == TREND_UP
                                ? newerSwingPrice < swingPrice
                                : newerSwingPrice > swingPrice;
      if(structureConfirmed)
        {
         stopLoss = NormalizePrice(newerSwingPrice);
         return true;
        }
      newerSwingPrice = swingPrice;
     }
...
bool OpenTrade(TrendDirection direction, bool pullbackEntry = false)
  {
   if(!AtrSideEntryAvailable(direction) || !TradeDirectionAllowed(direction) ||
      !g_stateAvailable || !AtrLineFilterPassed(g_state) ||
      !EmaGapFilterPassed(g_state) ||
   ...
   string comment = pullbackEntry
                    ...
                    : (direction == TREND_UP ? "TF aligned BUY"
                                             : "TF aligned SELL");
   ...
   if(direction == TREND_UP)
      g_buyAtrSideConsumed = true;
   else
      g_sellAtrSideConsumed = true;
...
//+------------------------------------------------------------------+
void PlaceManualButtonOrder(const bool buyOrder)
  {
   double volume = NormalizeVolume(InpLots);
   double entry = buyOrder ? SymbolInfoDouble(_Symbol, SYMBOL_ASK)
                           : SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double takeProfit = 0.0;
   if(g_stateAvailable && g_state.atr > 0.0)
      takeProfit = NormalizePrice(
         buyOrder
         ? entry + InpTakeProfitAtrMultiplier * g_state.atr
         : entry - InpTakeProfitAtrMultiplier * g_state.atr
      );
   ...
   if(!opened)
   ...
   else
     {
      Print("Manual ", buyOrder ? "BUY" : "SELL", " opened; volume=",
            DoubleToString(volume, 2), ", TP=",
            takeProfit > 0.0 ? DoubleToString(takeProfit, _Digits) : "none");
     }
  }

//+------------------------------------------------------------------+
void CloseButtonPositions(const long positionType)
  {
   for(int index = PositionsTotal() - 1; index >= 0; index--)
     {
      ulong ticket = PositionGetTicket(index);
...
bool HandleTradeButton(const string buttonName)
  {
   if(buttonName == BUTTON_BUY)
      PlaceManualButtonOrder(true);
   else if(buttonName == BUTTON_SELL)
      PlaceManualButtonOrder(false);
   else if(buttonName == BUTTON_CLOSE_BUY)
      CloseButtonPositions(POSITION_TYPE_BUY);
   else if(buttonName == BUTTON_CLOSE_SELL)
      CloseButtonPositions(POSITION_TYPE_SELL);
   else
      return false;
...
void CreateTradeButtons()
  {
   if(!InpShowPanel)
      return;
   int y = InpYOffset + (InpFontSize + 8) * 15;
   CreateTradeButton(BUTTON_BUY, "BUY", InpXOffset, y, 90, clrForestGreen);
   CreateTradeButton(BUTTON_SELL, "SELL", InpXOffset + 98, y, 90, clrFireBrick);
   CreateTradeButton(BUTTON_CLOSE_BUY, "CLOSE BUY", InpXOffset + 196, y,
                     130, clrDarkSlateGray);
   CreateTradeButton(BUTTON_CLOSE_SELL, "CLOSE SELL", InpXOffset + 334, y,
                     130, clrDarkSlateGray);
  }
...
void DrawPanel()
  {
   ...
   SetBackground(PREFIX + "bg", x - 8, y - 8, 560, lineHeight * 17 + 14);
   CreateTradeButtons();

   if(!g_stateAvailable)
     {
      SetLabel(PREFIX + "title", "MARKET STRUCTURE EA", x, y, clrWhite, InpFontSize + 1);
      SetLabel(PREFIX + "l1", TimeframeText() + " trade:      " + H1AtrAllowedDirectionText(),
   ...
   bool tradeReady = directionalTrend &&
                      ...
                      AtrSideEntryAvailable(g_state.trend) &&
   ...
   bool emaGapPassed = EmaGapFilterPassed(g_state);
   ...
   SetLabel(PREFIX + "l8", TimeframeText() + " close / ATR: " +
   ...
   SetLabel(PREFIX + "l13", TimeframeText() + " trade:      " + H1AtrAllowedDirectionText(),
            x, y + lineHeight * row++, clrAqua);
   SetLabel(PREFIX + "l14", "EMA gaps:     " +
            DoubleToString(MathAbs(g_state.fastEma - g_state.slowEma) /
                           g_state.atr, 2) + " / " +
            DoubleToString(MathAbs(g_state.slowEma - g_state.ema100) /
                           g_state.atr, 2) + " ATR; min > " +
            DoubleToString(InpMinimumEmaGapAtr, 2) + " [" +
            (emaGapPassed ? "PASS" : "FAIL") + "]",
            x, y + lineHeight * row++, emaGapPassed ? clrLimeGreen : clrOrange);
   ChartRedraw();
  }
...
void ProcessSignal()
  {
   ...
   RearmEntriesAfterAtrSideSwap(state);
   DrawPanel();
...
int OnInit()
  {
   ...
   if(!g_isTester)
     {
      Print("backtester_market_strucutre.mq5 runs only in MT5 Strategy Tester.");
      return INIT_FAILED;
     }
   ...
   // Draw separate chart objects so each EMA keeps its configured colour in
   // visual Strategy Tester runs (built-in tester plots use one default colour).
